...
export type POStatus = 'NEW' | 'CONFIRMED' | 'PROCESSING' | 'SHIPPED' | 'DELIVERED' | 'CANCELLED' | 'REFUNDED';
...
const productNames = [
  'Laptop Computer', 'Desktop Monitor', 'Wireless Mouse', 'Mechanical Keyboard',
  'Office Chair', 'Standing Desk', 'Printer Paper', 'Toner Cartridge',
  'USB Cable', 'HDMI Adapter', 'External Hard Drive', 'Cloud Storage License',
  'Software Subscription', 'Technical Support', 'Consultation Service', 'Training Package',
];

const streets = ['Nguyễn Huệ', 'Lê Lợi', 'Trần Hưng Đạo', 'Võ Văn Tần', 'Hai Bà Trưng', 'Lý Thường Kiệt'];
...
const generateFakePOs = async (): Promise<PurchaseOrder[]> => {
  ...
  return Array.from({ length: 50 }, (_, index) => {
    ...
    const status = randomElement<POStatus>(['NEW', 'CONFIRMED', 'PROCESSING', 'SHIPPED', 'DELIVERED', 'CANCELLED']);
    ...
    if (status === 'DELIVERED') {
      ...
      po.completedDate = new Date((po.deliveryDate || orderDate).getTime() + deliveredDaysAfter * 24 * 60 * 60 * 1000);
    }
...
export const purchaseOrderService = {
  ...
  async createPO(data: Omit<PurchaseOrder, 'id' | 'createdAt' | 'updatedAt'>): Promise<PurchaseOrder> {
