      ...
      <Accordion defaultValue="info" variant="contained">
          ...
          <Accordion.Panel>
            <Stack gap="xs">
              {purchaseOrder.items.map((item) => (
                <Group key={item.id} justify="space-between" p="xs" style={{ borderBottom: '1px solid var(--mantine-color-gray-3)' }}>
                  <div>
                    <Text size="sm" fw={500}>{item.description}</Text>
                    <Text size="xs" c="dimmed">{item.productCode}</Text>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <Text size="sm">{item.quantity} × {formatCurrency(item.unitPrice)}</Text>
                    <Text size="xs" fw={500}>{formatCurrency(item.totalPrice)}</Text>
                  </div>
                </Group>
              ))}
            </Stack>
        ...
        {purchaseOrder.shippingAddress && (
          <Accordion.Item value="address">
            ...
            <Accordion.Panel>
              <Text size="sm">
                {purchaseOrder.shippingAddress.street}<br />
                {purchaseOrder.shippingAddress.city}
                {purchaseOrder.shippingAddress.state && `, ${purchaseOrder.shippingAddress.state}`}
                {purchaseOrder.shippingAddress.postalCode && ` ${purchaseOrder.shippingAddress.postalCode}`}<br />
