...
export function POItemsEditor({
  ...
  const handleAddItem = () => {
    if (!newItem.productCode || !newItem.description || !newItem.quantity || newItem.unitPrice === undefined) {
      return;
    }
      ...
      <Table withTableBorder withColumnBorders>
        ...
        <Table.Tbody>
          {items.map((item) => (
            <Table.Tr key={item.id}>
              ...
              <Table.Td>
                <Text size="sm" ta="right">{formatCurrency(item.totalPrice)}</Text>
              </Table.Td>
              <Table.Td>
                <ActionIcon
                  color="red"
          ...
          <Table.Tr>
            ...
            <Table.Td>
              <NumberInput
                ...
                onChange={(value) => setNewItem({...newItem, quantity: typeof value === 'number' ? value : 1})}
                min={1}
                disabled={disabled}
              />
            </Table.Td>
            <Table.Td>
              <NumberInput
                ...
                onChange={(value) => setNewItem({...newItem, unitPrice: typeof value === 'number' ? value : 0})}
                min={0}
                thousandSeparator=","
                disabled={disabled}
              />
            ...
            <Table.Td>
              <NumberInput
                ...
                onChange={(value) => setNewItem({...newItem, discount: typeof value === 'number' ? value : 0})}
                min={0}
                max={100}
                suffix="%"
                disabled={disabled}
        ...
        <Table.Tfoot>
          <Table.Tr>
            ...
            <Table.Td>
              <Text fw={600} ta="right">{formatCurrency(subtotal)}</Text>
            </Table.Td>
            <Table.Td></Table.Td>
          </Table.Tr>
        </Table.Tfoot>
